...
struct MMNode
{
   ...
   int flag;
	 int pfAcols, pfAblk, pfBcols, pfCelts;
   ATL_mmnode_t *next;
};
...
static void PrintMMLine(FILE *fpout, ATL_mmnode_t *np)
{
   ...
   if (i > 70) { fprintf(fpout, " \\\n   "); i = 3; }
   i += fprintf(fpout, "PFBCOLS=%d ", np->pfBcols);
   if (i > 70) { fprintf(fpout, " \\\n   "); i = 3; }
   i += fprintf(fpout, "PFABLK=%d ", np->pfAblk);
   if (i > 70) { fprintf(fpout, " \\\n   "); i = 3; }
   i += fprintf(fpout, "PFACOLS=%d ", np->pfAcols);
   if (i > 70) { fprintf(fpout, " \\\n   "); i = 3; }
   i += fprintf(fpout, "PFCELTS=%d ", np->pfCelts);
   if (i > 70) { fprintf(fpout, " \\\n   "); i = 3; }
